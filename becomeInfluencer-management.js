...
function showPage(page_num)
{
  ...
  {
    table_body_html+="<tr>";
    var tmpEn = newJSON.influencers[i].status;
    var tmpState;
    switch(tmpEn) {
      case "Reviewing":
            tmpState = "Reviewing 审核中";
            break;
      case "Unprocessed":
            tmpState = "Unprocessed 未审核";
            break;
      case "Declined":
            tmpState = "Declined 拒绝";
            break;
      case "Accepted":
            tmpState = "Accepted 接受";
            break;
    }
    table_body_html+="<td>"+newJSON.influencers[i].first_name+" "+newJSON.influencers[i].last_name+"<\/td><td>"+newJSON.influencers[i].country+"<\/td><td>"+newJSON.influencers[i].apply_time+"<\/td><td>"+tmpState+"<\/td><td><button class=\"btn btn-link\" data-toggle=\"modal\" data-target=\"#myModal\" id='editbutton"+newJSON.influencers[i].id+"' onclick=\"viewandedit(event)\">More 更多详情<\/button><\/td>";
    table_body_html+="</tr>";
  }
...
{
  if(confirm("Are you sure to save? 确定保存吗?") == false)
    return;
  jQuery.ajax({
    type:"GET",
    url:"getallinfluencers.php",
