...
function getEventEntry(id,name, timeBegin, timeEnd, briefIntro ){
    return "<td>" + name + "</td>" +
        ...
        "<td>" + timeEnd + "</td>" +
        "<td>" + briefIntro + "</td>" +
        "<td><button class=\"btn btn-link\" id=\"editbutton"+id+"\" data-toggle=\"modal\" data-target=\"#myModal\" onclick=\"viewandedit(event)\">view&amp;edit 查看并编辑</button></td>" +
        "<td>" + id + "</td>";
}
...
function getChunk(imgSrc, mainTitle, subTitle, link){
    ...
    "Main Title 大标题: <input type=\"text\" class=\"entry_input\" value=\""+ mainTitle +"\">" +
    "Subtitle 副标题: <input type=\"text\" class=\"entry_input\" value=\""+ subTitle +"\">" +
    "Link 链接: <input type=\"text\" class=\"entry_input\" value=\""+ link +"\">" +
    "<button class=\"remove_chunk_buttons\" onclick=\"closestDiv()\">Remove 删除</button>" +
    "</div>";
}
...
function readFromJson(eventsJsonObj){
    ...
    jQuery("#tag_pool").html("<button id=\"add_tag_button\" class=\"btn btn-primary\" onclick=\"addTag()\">Add 添加</button>");
